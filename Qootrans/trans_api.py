...
class Trans_api:
    def __init__(self, setting):
        ...
        self.papago_free_keys = list(setting["api_keys"]["papago_free_api"].keys())
        self.papago_paid_keys = list(setting["api_keys"]["papago_paid_api"].keys())
        self.papago_free_values = list(setting["api_keys"]["papago_free_api"].values())
        self.papago_paid_values = list(setting["api_keys"]["papago_paid_api"].values())
        self.papago_range = range(len(self.papago_free_keys) + len(self.papago_paid_keys))
        
        self.papago_url_dic = {"free":["https://openapi.naver.com/v1/papago/n2mt","X-Naver-Client-Id","X-Naver-Client-Secret",self.papago_free_keys,self.papago_free_values],
                               "paid":["https://naveropenapi.apigw.ntruss.com/nmt/v1/translation","X-NCP-APIGW-API-KEY-ID","X-NCP-APIGW-API-KEY",self.papago_paid_keys,self.papago_paid_values]}
        ...
        #경우의 수가 네가지.
        #1. 무료 api 키가 있고, 유료 api 키가 있는 경우
        #2. 무료 api 키가 있고, 유료 api 키도 없는 경우
        #3. 무료 api 키가 없고, 유료 api 키가 있는 경우
        #4. 무료 api 키가 없고, 유료 api 키도 없는 경우
        #조건: 무료 api 키를 우선적으로 사용한다.
        if len(self.papago_free_keys) == 0 and len(self.papago_paid_keys) != 0:#무료 api 키가 없고, 유료 api 키가 있는 경우
            self.log.info("무료 키가 없어요. 대신, 파파고 유료 api 키를 사용할게요..")
            self.papago_flag = "paid"
        elif len(self.papago_free_keys) == 0 and len(self.papago_paid_keys) == 0:
            #무료 api 키가 없고, 유료 api 키가 없는 경우
            self.log.info("파파고 api 키가 없어요. 다른 api를 사용해야 돼요..")
            self.papago_flag = "none"
        else:
            self.log.info("파파고 무료 api 키를 사용할게요..")
            self.papago_flag = "free"
        ...
        self.google_dic = {"한국어":"ko","일본어":"ja","영어":"en","중국어 간체":"zh-CN", "중국어 번체":"zh-TW","포르투갈어":"pt", "스페인어":"es", "프랑스어":"fr", "독일어":"de", "베트남어":"vi", "튀르키예어":"tr",
                             ...
                             "벵골어":"bn","보지푸리어":"bho","불가리아어":"bg","카탈루냐어":"ca","세부아노어":"ceb", "코르시카어":"co","크로아티아어":"hr","체코어":"cs","덴마크어":"da","디베히어":"dv","도그리어":"doi","네덜란드어":"nl","에스페란토어":"eo",
    ...
    def papago_api(self, source_txt):#클립보드 번역용
        ...
        self.power_swich = True
        while self.power_swich == True:#파파고 api 사용량 초과시, 무료 api키로 전환하기 위한 반복문.
            request = urllib.request.Request(self.papago_url_dic[self.papago_flag][0])#url 입력
            request.add_header(self.papago_url_dic[self.papago_flag][1], self.papago_url_dic[self.papago_flag][3][self.papago_index])#헤더와 api키 입력
            request.add_header(self.papago_url_dic[self.papago_flag][2], self.papago_url_dic[self.papago_flag][4][self.papago_index])
            try:
                response = urllib.request.urlopen(request, data=data.encode("utf-8"))
            except Exception as e:
                self.log.info(e)
                if "HTTP Error 429" in str(e):#api 사용량 초과

                    if self.papago_flag == "free":
                        if self.papago_index < len(self.papago_free_keys):
                            self.papago_index += 1
                            continue#무료키가 아직 있는 경우, 다시 시도
                        else:
                            self.log.info("무료 api 키가 모두 사용됐어요.")
                            if len(self.papago_paid_keys) > 0:
                                self.log.info("대신 유료 api 키를 사용할게요.")
                                self.papago_flag = "paid"
                                self.papago_index = 0
                                continue#유료키로 다시 시도
                            else:
                                self.log.info("유료 api 키가 없어요. 장비를 정지할게요.")
                                self.power_swich = False
                                quit()#긴급탈출
                            
                    elif self.papago_flag == "paid" and self.papago_index < len(self.papago_paid_keys):
                        self.papago_index += 1
                        continue#다른 유료 키가 아직 있는 경우, 다시 시도

                    
                else:
                    self.log.info("알 수 없는 오류 발생했어요 X_X")
                    self.power_swich = False
                    quit()#긴급탈출
            ...
            if rescode == 200:
                ...
                self.log.info(translated_text)
                return translated_text
            else:
                self.log.info("Error Code:" + rescode)
                self.power_swich = False
                quit()#긴급탈출
