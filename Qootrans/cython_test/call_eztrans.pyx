...
cpdef clip_trans(str text):  # 클립보드 번역용
    try:
        return eng.trans(0, "".join(["|:_", text])).lstrip("|:_")
    except Exception as e:
        return str(e)

cdef readfile(txt_path):#내부함수
    with open(txt_path, "r", encoding="utf-8") as f:
        return (f.readlines(), txt_path)

cdef writefile(str save_path, object txts):#내부함수
    with open(save_path, "w", encoding="utf-8") as f:
        f.writelines(txts.get())
        print(f"{save_path} 완료")

cpdef main(a22):  # 텍스트 파일들 번역용, 텍스트 파일 목록만 전달 받도록 함.
    path = Path(a22)
    cdef dict a24
