...
if __name__ == "__main__":
    freeze_support()
    print("Qootrans CLI ver.0.3 !@!번역할 텍스트는 utf-8 인코딩, .txt 확장자여야 합니다!@!")

    if len(sys.argv) == 1:
        # 인자가 없으면 실행됨.
        print("Qoo_cli file_mode (번역할 텍스트가 있는 폴더 경로) <-이런 식으로 입력")
    else:
        # 인자가 있으면 실행.
        try:
    ...
    # 또, chunksize를 크게 하면 더 빠를까? - 1로 해본 결과 더 느림(14395ms). 10000으로 해도 느림.
    # 그리고 프로세스 풀 안에 멀티쓰레드를 하면 어떨까? - 되긴 되나 매우 느려짐..
    ...
    # 05/01 속도 테스트 용 - 303kb로 실험. - 15557ms - 15.55초
        ...
        //14.64초-as_completed사용//14.53초//14.48초(리스트컴프리 극한까지 씀.)//14.69초
        //nuitka로 컴파일. - 13.79초.
        # 15557ms에서 13792ms로 줄었으니 11.4% 정도 빨라짐.
        # 대량으로 번역했을 때 테스트 필요. - 7.47mb로 실험. 기존 코드: 249633ms-249.63초
        // 0503 11:30 코드(map사용) - 253220ms - 253.22초
        // 13:18 코드(map_async사용) - 228720ms - 228.72초
        // 13:27 코드(imap사용) - 231317ms - 231.32초
        // 0504(nuitka) 224795ms - 224.79초
        # 249633ms에서 224795ms로 줄었으니 10% 정도 빨라짐.

    # 이번에 개선한 점
            # 1.번역할 파일들 읽기 속도 개선.
            # 2.번역할 파일이 많을 때 작업 속도 개선.
            # 3.단일 텍스트 파일 번역 기능 추가.
            # 0.2버전과 비교해 속도 개선률은 10% 정도.
            # 파이썬 성능을 극한까지 쥐어짜냈다고 자부할 수 있음. 여기서 더 빠르게 하려면, C++로 만들어야 함.
            # 기존 방법. 파일 하나 읽고 그 파일의 텍스트를 멀티프로세스로 번역하고 저장하고. 그리고 다음 파일 읽고 번역하고 저장하고. 이런 식.
            # 개선된 방법. 여러 텍스트 파일을 한꺼번에 빠르게 읽고 다 읽은 파일부터 프로세스들에게 줌. 그리고 번역한 다음 저장함.
    """
