...
def main():
    ...
    tsv_list = []#tsv 파일에 chr, cha 열이 하나가 아닌 경우, 그런 열이 들어간 번역 파일은 다시 불러오지 않도록 한다. 그것을 위한 리스트
    for list in file_list:
        ...
        if qq != None:
            if ".tsv_" in qq.group():#tsv 번역 파일 관련 줄
                ...
                df = pd.read_csv(red, sep = '\t', encoding = 'CP949', encoding_errors='surrogateescape')
                print('tsv파일: '+red)
                #chr, cha로 된 열 감지.
                ...
                for koul in col_list:
                    if koul not in list:#tsv파일에 chr, cha가 하나가 아닌 경우, 해당 열의 번역파일도 불러오는데, 그것을 나중에 다시 불러오는 걸 막기 위해, 리스트로 저장.
                        list = re.sub('(?<=.tsv)_.+?(_번역.txt)',r'_'+koul+r'\1',list)
                    ...
                    try:
                        ...
                        if "[Errno 2] No such file or directory:" in str(value):#이 에러가 발생되는 건, 추출할 때, 빈 chr,cha열은 그 열의 이름으로 된 텍스트파일을 저장하지 않기 때문.
                            for number, ju in enumerate(df[koul]):
                                if True != pd.isna(ju):#빈칸이 아닌 칸이 하나라도 나올 경우
                                    print('추출과정에서 뭔가 잘못됐네요. 멈춥니다.')#추출할 때, 모든 줄을 완벽하게 추출했다면 이런 경우는 발생하지 않음.
                                    os.system('pause')
                ...
                df.to_csv(red,sep='\t',index=False,encoding = 'cp949',errors='surrogateescape')
            ...
            if qq.group() == ".ext_번역.txt":#txt 번역 파일 관련 줄
                ...
                for i, line in enumerate(r1):
                    if "[EV_OP:01]" in line:
                        t = uyt1[y].replace('\n','')
                        k = re.sub('(?<=\[◆\]).+?(?=\[◆\])',t,line)#대사 검색 후 번역문으로 교체
                        y += 1
