...
def resource_path(relative_path):#pyinstall용 ui파일 불러오지 못하는 문제. https://editor752.tistory.com/140
...
class livemaker(QThread):#쓰레드 구현
    #초기화 메서드 구현    
    def __init__(self, parent, text): #parent는 WndowClass에서 전달하는 self이다.(WidnowClass의 인스턴스)
    ...
    def detect(self):
        list1 = os.listdir()
        if ('game.dat' not in list1) and ('game.ext' not in list1):#game.dat, game.ext가 있는지 감지.
            #dat 파일이 없다면 exe 파일 안에 통짜로 되어 있다는 말이니까. 
            for e in list1:
                if ('.exe' in e) and ('ToolForLiveMaker.exe' != e) and ('_pure.exe' not in e) and ('_pure_h.exe' not in e) and ('_new.exe' not in e) and ('ToolForLiveMakerGUI.exe' != e):#기존 작업 파일 제외
                    self.exe_game = e#game.exe에서 감지된 게임 실행파일명(死月妖花～四月八日～.exe)으로 바꿈.

    def lazer(self, gg):#툴오브 라이브메이커 콘솔출력용https://stackoverflow.com/questions/2804543/read-subprocess-stdout-line-by-line  
            for line in iter(gg.stdout.readline,''):
                print(line.rstrip())

            for line2 in iter(gg.stderr.readline,''):
                print(line2.rstrip())
    ...
    def run(self):
        ...
        if self.text == "unpack":
            ...
            list1 = os.listdir()
            if ('game.dat' in list1) and ('game.ext' in list1):#game.dat, game.ext가 있는지 감지.
                game_list = "game.dat"
                game_list2 = []
                for li in list1:
                ...
                if len(game_list2) != 0:#game.001,game.002 같은 파일이 없을 경우 이 줄은 넘어간다.
                    os.system('copy /b '+game_list+' game_merged.dat')#dat 파일 합치기.
            ...
            self.a = subprocess.Popen(im, bufsize = 0, shell = True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True, encoding="cp949", errors="ignore")
            self.lazer(self.a) 
            self.a.wait()
            self.a.kill()
            #proc = subprocess.Popen(im, shell = True, stdout=subprocess.PIPE, universal_newlines=True, encoding="utf-8")
        ...
        if self.text == "locale":
            ...
            self.a = subprocess.Popen('ToolForLiveMaker.exe -cvte '+self.exe_game, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell = True, universal_newlines=True, encoding="cp949", errors="ignore")
            self.lazer(self.a) 
            self.a.wait()
            self.a.kill()
            #os.system('ToolForLiveMaker.exe -cvte '+self.exe_game)#pure_exe파일 한국어 인코딩으로 전환
            
            self.b = subprocess.Popen('ToolForLiveMaker.exe -cvt Output', shell = True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True, encoding="cp949", errors="ignore")
            self.lazer(self.b) 
            self.b.wait()
            self.b.kill()
        ...
        if self.text == "extract":
            ...
            self.a = subprocess.Popen('ToolForLiveMaker.exe -extt --code -kor Output', shell = True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True, encoding="cp949", errors="ignore")
            self.lazer(self.a) 
            self.a.wait()
            self.a.kill()
        ...
        if self.text == "insert":
            ...
            self.a = subprocess.Popen('ToolForLiveMaker.exe -rept Output', shell = True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True, encoding="cp949", errors="ignore")
            self.lazer(self.a) 
            self.a.wait()
            self.a.kill()
        ...
        if self.text == "repack":
            ...
            for path, dirs, files in os.walk('Output'):#추출한 텍스트 파일 삭제
                ...
                for file in files:
                    if ".ext.txt" in file or "_번역.txt" in file:#.ext가 붙지 않는 텍스트 파일도 있지만, 혹시 몰라 지우지 않음.
            ...
            if j == 0:
                    ...
                    self.a = subprocess.Popen('ToolForLiveMaker.exe -p --onto '+self.exe_game+' Output', shell = True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True, encoding="cp949", errors="ignore")
                    self.lazer(self.a) 
                    self.a.wait()
                    self.a.kill()
            ...
            if j == 1:
                    ...
                    self.a = subprocess.Popen('ToolForLiveMaker.exe -p Output', shell = True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True, encoding="cp949", errors="ignore")
                    self.lazer(self.a) 
                    self.a.wait()
                    self.a.kill()
            ...
            if j == 2:
                    ...
                    self.a = subprocess.Popen('ToolForLiveMaker.exe -p --onto '+self.exe_game+' Output -LowVer', shell = True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True, encoding="cp949", errors="ignore")
                    self.lazer(self.a) 
                    self.a.wait()
                    self.a.kill()
            ...
            if j == 3:
                    ...
                    self.a = subprocess.Popen('ToolForLiveMaker.exe -p Output -LowVer', shell = True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True, encoding="cp949", errors="ignore")
                    self.lazer(self.a) 
                    self.a.wait()
                    self.a.kill()
