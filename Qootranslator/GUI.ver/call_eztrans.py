...
def main(a22):  # 텍스트 파일들 번역용, 텍스트 파일 목록만 전달 받도록 함.
    ...
    with multiprocessing.Pool() as pool, concurrent.futures.ThreadPoolExecutor() as thread:
        """
        for texts in thread.map(readfile, a24):
            pre_texts.extend(texts)
        """
        for texts, file_path in zip(thread.map(readfile, a24), save_paths):  # thread.map으로 파일을 읽어오고 save_paths순으로 텍스트덩어리가 나열됨.
            thread.submit(writefile, file_path, pool.imap(clip_trans, texts))
            ...
            # 이번에 개선한 점
            # 1.번역할 파일 읽고, 번역된 파일 저장 속도 개선.
            # 2.텍스트 번역 속도 개선. 메모리 개선.
            # 기존 방법. 파일 하나 읽고 그 파일의 텍스트를 멀티프로세스로 번역하고 저장하고. 그리고 다음 파일 읽고 번역하고 저장하고. 이런 식.
            # 개선 방법. 여러 텍스트 파일을 한꺼번에 빠르게 읽고 하나의 커다란 텍스트 리스트를 만듬. 그걸 프로세스풀에 던짐. 그리고 프로세스들은 그걸 하나씩 꺼내서 작업함(아래세 추가설명).
            # asyncio을 이용해서 비동기 프로그래밍은 어떨까. 더 빠를까?
            # 번역이 완료되는 파일부터 저장.
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        ...
        main("새 폴더")
        # 05/01 속도 테스트 용 - 303kb로 실험. - 15557ms - 15.55초// 05/03 - 15242ms - 15.24초 2차 테스트 - 15052msms - 15.05초 3차 테스트 - 14773ms - 14.77초 4차 테스트 - 14487ms - 14.48초(imap씀) 5차 테스트 - 14287ms - 14.28초(비동기 방식 map_async)
        # 15557ms에서 14287ms로 줄었으니 8.2% 정도 빨라짐.
        # 대량으로 번역했을 때 테스트 필요. - 7.47mb로 실험. 0503 11:30 코드(map사용) - 253220ms - 253.22초// 13:18 코드(map_async사용) - 228720ms - 228.72초//
        input()  # 코드 정지용
        sys.exit()
