...
class Clipboard_Watch(QtCore.QThread):
    ...
    def __init__(self):
        super().__init__()
        hwnd1 = self._create_window()
        ctypes.windll.user32.AddClipboardFormatListener(hwnd1)  # 클립보드 감시자 등록
        self.running = True
    ...
    def run(self):
        print("클립보드 프로세스 시작")
        win32gui.PumpMessages()  # 얘가 무한 반복. - 어떻게 멈추는지 모르겠음.
    ...
    def pause(self, int):
        ...
        if int == 1:
            print("클립보드 프로세스 일시정지")
            self.terminate()  # 이 부분이 문제. terminate()를 호출했는데 종료가 안됨.
            # 5000ms = 5s
...
class clipwindow(QtWidgets.QMainWindow, form_class2):  # 클립보드를 번역한 텍스트를 보여주는 창
    signal2 = QtCore.pyqtSignal(int)  # 반드시 클래스 변수로 선언할 것
    ...
    def closeEvent(self, event):
        self.signal2.emit(1)  # 쓰레드 일시정지 보내는 신호- 안 하면 안 꺼지고 돌아가고 있음..
        print("클립보드 창 종료")
...
class mainWindow(QtWidgets.QMainWindow, form_class):
    ...
    def clipwindow(self):  # 클립보드를 번역한 텍스트를 보여주는 창
        ...
        self.clip.signal2.connect(self.clipboard.pause)
