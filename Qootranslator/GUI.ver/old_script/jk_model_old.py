...
class Clipboard_Watch:
    def __init__(self):
        encoder_model_name = 'best_model/src_tokenizer'
        decoder_model_name = "best_model/trg_tokenizer"
        self.device = "cuda:0" if torch.cuda.is_available() else "cpu"
        self.src_tokenizer = BertJapaneseTokenizer.from_pretrained(encoder_model_name)
        self.trg_tokenizer = PreTrainedTokenizerFast.from_pretrained(decoder_model_name)
        print('로딩 중')
        self.model = EncoderDecoderModel.from_pretrained('best_model').to(self.device)
        ...
        ##간단한 단어사전 추가.
        #전처리 사전
        with open('pre_dic.csv', 'r', encoding='utf-8-sig') as f:
            lines = csv.reader(f)
            for line in lines:
                self.pre_dic[line[0]] = line[1]#{수정전단어 : 수정후단어}

        #후처리 사전
        with open('post_dic.csv', 'r', encoding='utf-8-sig') as f:
            lines = csv.reader(f)
            for line in lines:
                self.post_dic[line[0]] = line[1]#{수정전단어 : 수정후단어}
        #print(dic)
        #quit()
        #どうだろう？　小鳥ちゃん見て顔が引きつってるの、むしろ藍城さんの方だし……」

        #ギルガメッシュ討伐戦に行ってきます。 
        #text = "ギルガメッシュ討伐戦に行ってきます。一緒に行きましょうか？"
        print('대기')

    def trans(self, text):#번역 함수
            embeddings = self.src_tokenizer(text, return_attention_mask=False, return_token_type_ids=False, return_tensors='pt').to(self.device)
            embeddings = {k: v for k, v in embeddings.items()}
            output = self.model.generate(**embeddings, max_new_tokens=2000)[0, 1:-1]
            a = self.trg_tokenizer.decode(output.cpu())
            return a
