...
class recognizion:
        ...
        for self.file_path in self.file_dics.keys():
            ...
            if self.stt_method[0] == 0:#whisper-
                if self.stt_method[1] == 0:#whisper-auto
                    self.whisper_exe(language=None, file_path = self.audio_path, device=self.device)
                if self.stt_method[1] == 1:#whisper-korean
                    self.whisper_exe(language="ko", file_path = self.audio_path, device=self.device)
                if self.stt_method[1] == 2:#whisper-english
                    self.whisper_exe(language="en", file_path = self.audio_path, device=self.device)
                if self.stt_method[1] == 3:#whisper-japanese
                    self.whisper_exe(language="ja", file_path = self.audio_path, device=self.device)
                if self.stt_method[1] == 4:#whisper-chinese
                    self.whisper_exe(language="zh", file_path = self.audio_path, device=self.device)
    ...
    def load_sub(self, sub_path):#자막 파일 로드.
        ...
        for self.time_interval in self.time_intervals:
            ...
            self.audio_segment = [self.time_interval[0], self.time_interval[1], self.audio_array[int(int(self.time_interval[0]) * sr):  int(int(self.time_interval[1]) * sr)]]
    ...
    #위스퍼 모델 실행
    def whisper_exe(self, language, file_path, device):#language = None이면 위스퍼가 auto로 진행됨.
...
#todo - liblosa를 이용해 오디오 타임스탬프가 있는 자막을 이용해 음성 구간을 추출해서 텍스트 추출하는 기능 만들 것. - 함. 근데 별 쓸모는 없는 듯.
